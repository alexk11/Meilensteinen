...
export class OrderSearchSearchComponent implements OnInit {
    ...
    private orderSearchForm = this.fb.group({
        inOrderId: [''],
        inOrderPosId: [''],
        inCustomerId: [''],
    });

    private regexOrderId = '^([1-9]-[0-9]{5,20})$|(K{2}([UV]))([0-9]{5,20})$';
    private regexOrderPosId = '^(([1-9]-[0-9]{5,34})|(k{2}([uv]))([0-9]{5,34}))-[0-9]{6}$';
    private regexCustomerId = '^([1-9][0-9]{9})$';
    ...
    private onOptionsChanged(searchType: SearchType) {
